...
plt.xlabel('成分1', fontname="HGMinchoB") #X軸のラベル fontname：日本語の文字化け防止にフォントを指定
plt.ylabel('成分2', fontname="HGMinchoB") #Y軸のラベル
...
for label,x,y in zip(labels,rws[:,0],rws[:,1]):
    plt.annotate(label, xy=(x, y), fontname="HGMinchoB",color = 'b')
...
for label,x,y in zip(labels,cols[:,0],cols[:,1]):
    plt.annotate(label, xy=(x, y), fontname="HGMinchoB",fontsize=8,color = 'c')
