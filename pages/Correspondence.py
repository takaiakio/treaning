...
plt.xlabel('成分1', fontname='MS Mincho') #X軸のラベル fontname：日本語の文字化け防止にフォントを指定
plt.ylabel('成分2', fontname='MS Mincho') #Y軸のラベル
...
for label,x,y in zip(labels,rws[:,0],rws[:,1]):
    plt.annotate(label, xy=(x, y), fontname='MS Mincho',color = 'b')
...
for label,x,y in zip(labels,cols[:,0],cols[:,1]):
    plt.annotate(label, xy=(x, y), fontname='MS Mincho',fontsize=10,color = 'c')
